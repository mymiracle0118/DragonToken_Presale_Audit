...
describe("QuestRouter02 contract", function () {
  async function deployRouterFixture() {
    const [owner, addr1, addr2] = await ethers.getSigners();

    const wavax = await ethers.deployContract("WAVAX");
    await wavax.waitForDeployment();
    const wavaxAddress = await wavax.getAddress();

    const questFactory = await ethers.deployContract("QuestFactory", [owner.address]);
    await questFactory.waitForDeployment();
    const factoryAddress = await questFactory.getAddress();

    const router = await ethers.deployContract("QuestRouter02", [factoryAddress, wavaxAddress]);
    ...
    await myToken.approve(routerAddress, '88888888000000000000000000');
    factoryCheck = await router.factory();

    const hash = await questFactory.pairCodeHash();
    console.log("pairCodeHash", hash);

    const depositOne = ethers.parseEther("1.0");
    await router.addLiquidityAVAX(
        myTokenAddress,
        '80000000000000000000000000', // tokenAmountDesired
        '80000000000000000000000000', // tokenAmountMin
        depositOne,         // ethAmountMin
        owner.address,
        '1810673239', // expiration
        { value: depositOne, from: owner.address }
    );  
    
    const myOtherToken = await ethers.deployContract("Token");
    await myOtherToken.waitForDeployment();
    const myOtherTokenAddress = await myOtherToken.getAddress();

    const dragonFire = await ethers.deployContract("DragonFire");
    await dragonFire.waitForDeployment();
    const dragonFireAddress = await dragonFire.getAddress();

    const phaseBefore = await dragonFire.tradingPhase();
    console.log("phaseBefore", phaseBefore);
    await dragonFire.approve(dragonFireAddress, '88888888000000000000000000');
    await myToken.approve(dragonFireAddress, '88888888000000000000000000');
    await dragonFire.seedAndBurnDragonLP('888888000000000000000000', depositOne, { value: depositOne, from: owner.address });
    await dragonFire.seedAndBurnCtLP(0, myTokenAddress, '888888000000000000000000', '888888000000000000000000');
    await dragonFire.setAllowlistedForSomePhase([owner.address, addr1.address, addr2.address], 1);
    

    // Perform a transaction and get the block timestamp
    const tx = await dragonFire.setPairs();
    const receipt = await tx.wait();
    const blockNumber = receipt.blockNumber;
    const block = await ethers.provider.getBlock(blockNumber);
    const timestamp = block.timestamp;
    console.log("timestamp", timestamp)
    await dragonFire.setWhaleLimitsPerPhase(['100000000000000000000','200000000000000000000','300000000000000000000','400000000000000000000','500000000000000000000','600000000000000000000','700000000000000000000'])


    await dragonFire.setPhasesStartTime(timestamp+3);
    // Perform a transaction and get the block timestamp
    const tx1 = await dragonFire.lockPhasesSettings();
    ...
    const phaseAfter = await dragonFire.tradingPhase();
    console.log("phaseAfter", phaseAfter);
  ...
  describe("Deployment", function () {
    ...
    it("Should print contract addresses", async function () {
        const {
            factoryCheck,
            owner,
            dragonFireAddress,
    ...
    expect(wavaxAddress).to.not.be.null;
    expect(factoryAddress).to.not.be.null;
    expect(routerAddress).to.not.be.null;
    expect(myTokenAddress).to.not.be.null;
    expect(myOtherTokenAddress).to.not.be.null;
    expect(dragonFireAddress).to.not.be.null;


    console.log("WAVAX contract address:", wavaxAddress);
    console.log("QuestFactory contract address:", factoryAddress);
    console.log("FactoryCheck contract address:", factoryCheck)
    console.log("QuestRouter02 contract address:", routerAddress);
    console.log("myTokenAddress contract address:", myTokenAddress);
    console.log("MyOtherToken contract address:", myOtherTokenAddress);
    console.log("dragonFireAddress contract address:", dragonFireAddress);
    console.log("owner address:", owner.address);
    });



    it("Should send process fees minimum worth of dragon from owner to the contract and process them", async function () {
        const { router, owner, dragonFire, dragonFireAddress } = await loadFixture(deployRouterFixture);
    
        // Get the address of Questpair contract
        const questpairAddress = await dragonFire.uniswapV2Pair();
    
        // Get the initial reserves of the LP tokens
        const Questpair = await ethers.getContractFactory("QuestPair"); // Replace "QuestPair" with the actual name of the contract
        const questpairInstance = await Questpair.attach(questpairAddress);
        const initialReserves = await questpairInstance.getReserves();
    
        // Calculate process fees minimum worth of dragon
        const processFeesMinimum = await dragonFire.processFeesMinimum();
    
        // Send process fees minimum worth of dragon from owner to the contract
        await dragonFire.transfer(dragonFireAddress, processFeesMinimum);
    
        // Verify dragon balance on router contract
        const routerDragonBalance = await dragonFire.balanceOf(dragonFireAddress);
        expect(routerDragonBalance).to.equal(processFeesMinimum);
    
        // Process the fees
        await dragonFire.processFees();
    
        // Verify that the dragon balance on the contract has decreased after processing fees
        const routerDragonBalanceAfter = await dragonFire.balanceOf(dragonFireAddress);
        expect(routerDragonBalanceAfter).to.lessThan(routerDragonBalance);
    
        // Call getReserves() again to get the updated reserves
        const updatedReserves = await questpairInstance.getReserves();
    
        // Verify that the LP tokens have increased
        expect(updatedReserves[0]).to.not.equal(initialReserves[0]);
        expect(updatedReserves[1]).to.not.equal(initialReserves[1]);
    });
    
    it("Should set up phases and trade and process fees on sells", async function () {
        const { wavaxAddress, addr1, router, routerAddress, owner, dragonFire, dragonFireAddress } = await loadFixture(deployRouterFixture);
        ...
        // Swapping AVAX for tokens and selling tokens for AVAX for the owner
        await router.swapAVAXForExactTokens('100000000000000000000', [wavaxAddress, dragonFireAddress], owner.address, '999992112234', 
            { value: '1000000000000000000', from: owner.address}); 
        
        await dragonFire.approve(routerAddress, '88888888000000000000000000');

        await router.swapExactTokensForAVAXSupportingFeeOnTransferTokens('92000000000000000000', 100, [dragonFireAddress, wavaxAddress], owner.address, '999992112234');

        // Send process fees minimum worth of dragon from owner to the contract
        await dragonFire.transfer(dragonFireAddress, processFeesMinimum);

        // Swapping AVAX for tokens and selling tokens for AVAX for addr1
        await router.connect(addr1).swapAVAXForExactTokens('100000000000000000000', [wavaxAddress, dragonFireAddress], addr1.address, '999992112234', 
            { value: '1000000000000000000', from: addr1.address });

        await dragonFire.connect(addr1).approve(routerAddress, '88888888000000000000000000', { from: addr1.address });

        await router.connect(addr1).swapExactTokensForAVAXSupportingFeeOnTransferTokens('92000000000000000000', 100, [dragonFireAddress, wavaxAddress], addr1.address, '999992112234', 
            { from: addr1.address });

                  // Try to buy more than allowlisted for; `require` will evaluate false and revert the transaction.
        await expect(router.connect(addr1).swapAVAXForExactTokens('100000000000000000000', [wavaxAddress, dragonFireAddress], addr1.address, '999992112234', 
        { value: '1000000000000000000', from: addr1.address})).to.be.reverted;
...
//Hello, so the goal here is to write unit testing to test out the dragonFire contract.
//I would like your help to write the unit tests for the following functions:
//allFunctions[ 
//setFees, setExcludedFromFees, setCommunityLPFee, setLiquidityFee, setTreasuryFee, setFarmFee, setProcessFeesMinimum, setUniswapV2Router, setUniswapV2Routers, setCommunityTokens,
//setCtRouters, setMainDex, setTreasuryAddress, setFarmAddress, setFeesInBasisPts, setPhasesInitialized, setApprovedRouters, setDragonCtPairs, setAllowlisted, setMaxWeiPerPhase,
//setTotalFees, setStartTime, setLastTimeCalled, setCommunityLPFee, setLiquidityFee, setTreasuryFee, setFarmFee, setProcessFeesMinimum, setFeesLocked, setPhasesInitialized, setApprovedRouters,
//]
//allEvents[SwapAndLiquify, ProcessFees, SettingsChanged, IncludeInFees, ExcludeFromFees, AvaxWithdraw, TokenWithdraw, TokenApproved, NFTWithdraw]
//alliERC20Functions[approve, transfer, transferFrom]
//allIERC721Functions[transferFrom, safeTransferFrom, safeTransfer, transfer]

//Here is the dragonFire code for reference, sorry I had to turn it all into comments to show here, but I hope it helps:
/* $DRAGON Links:
...
contract DragonFire is ERC20, ERC20Permit, Ownable {
    ...
    address public constant WAVAX = 0xE5f2A565Ee0Aa9836B4c80a07C8b32aAd7978e22; //Remix VM 0xE5f2A565Ee0Aa9836B4c80a07C8b32aAd7978e22
...
*///        communityTokens = [0x9396B453Fad71816cA9f152Ae785276a1D578492]; //Remix VM 0x9396B453Fad71816cA9f152Ae785276a1D578492
...
*/
       // uint256 length_ = communityTokens.length; //Total number of Community Tokens
        ...
        //ctRouters = [0xEc29164D68c4992cEdd1D386118A47143fdcF142]; //Remix VM 0xEc29164D68c4992cEdd1D386118A47143fdcF142
...
/*      [0x60aE616a2155Ee3d9A68541Ba4544862310933d4,
        ...
        0x60aE616a2155Ee3d9A68541Ba4544862310933d4,
        0x60aE616a2155Ee3d9A68541Ba4544862310933d4,
        0x60aE616a2155Ee3d9A68541Ba4544862310933d4];    //(TraderQuest has best liquidity for each CT LP currently)
*/
/*        require(ctRouters.length == length_, "Each token in the communityTokens array must have a corresponding router in the ctRouters array");
        ...
        approvedRouters[0xEc29164D68c4992cEdd1D386118A47143fdcF142] = true; //Remix VM 
        approvedRouters[0x60aE616a2155Ee3d9A68541Ba4544862310933d4] = true; //TraderQuest router https://support.traderquestxyz.com/en/articles/6807983-contracts-api
        ...
        routerLP = 0xEc29164D68c4992cEdd1D386118A47143fdcF142; //Remix VM 0xEc29164D68c4992cEdd1D386118A47143fdcF142
        //Main Dragon/AVAX LP and DRAGON/CT LP dex router
        //TraderQuest router = C-Chain Mainnet: 0x60aE616a2155Ee3d9A68541Ba4544862310933d4 ; Fuji Testnet: 0xd7f655E3376cE2D7A2b08fF01Eb3B1023191A901
